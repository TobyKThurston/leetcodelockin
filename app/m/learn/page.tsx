          ...
          <section key={path.id} className="space-y-2.5">
            <div className="flex items-baseline justify-between gap-3">
              <h2
                ...
                style={SG}
              >
                <span className="text-slate-400 mr-2 font-semibold tabular-nums">
            ...
            <div className="bg-white/[0.02] ring-1 ring-white/[0.06] rounded-2xl divide-y divide-white/[0.04] overflow-hidden">
              {lessonBlocks.map((block) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center gap-3 px-4 py-3.5 min-h-[60px] hover:bg-slate-50/70 active:bg-slate-50 transition-colors"
                  >
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                        isDone
                          ? 'bg-emerald-500/10 ring-1 ring-emerald-500/25'
                          : 'bg-slate-50/70 ring-1 ring-white/[0.06]'
                      }`}
                    >
                      <BookOpen
                        size={14}
                        strokeWidth={2}
                        className={isDone ? 'text-emerald-300' : 'text-slate-600'}
                      />
                    </div>
                    ...
                    <ChevronRight size={15} strokeWidth={2.5} className="text-slate-400 shrink-0" />
                  </Link>
                );
              })}
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-100">
                    <Code2 size={12} strokeWidth={2} className="text-slate-400" />
